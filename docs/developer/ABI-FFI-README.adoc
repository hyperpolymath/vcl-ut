...
│       └── include/
│           └── vql_ut.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── vql_ut.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/vql_ut.h
```
...
int main() {
    void* handle = vql_ut_init();
    if (!handle) return 1;

    int result = vql_ut_process(handle, 42);
    if (result != 0) {
        const char* err = vql_ut_last_error();
...
#[link(name = "vql_ut")]
extern "C" {
    fn vql_ut_init() -> *mut std::ffi::c_void;
    fn vql_ut_free(handle: *mut std::ffi::c_void);
    fn vql_ut_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = vql_ut_init();
        assert!(!handle.is_null());

        let result = vql_ut_process(handle, 42);
        assert_eq!(result, 0);
...
function init()
    handle = ccall((:vql_ut_init, libvql_ut), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:vql_ut_process, libvql_ut), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:vql_ut_free, libvql_ut), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/vql_ut.h
   ```
