...
# VCL_TOTAL ABI/FFI Documentation
...
│       └── include/
│           └── vcl_total.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── vcl_total.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/vcl_total.h
```
...
int main() {
    void* handle = vcl_ut_init();
    if (!handle) return 1;

    int result = vcl_ut_process(handle, 42);
    if (result != 0) {
        const char* err = vcl_ut_last_error();
...
Compile with:
```bash
gcc -o example example.c -lvql_ut -L./zig-out/lib
```

### From Idris2

```idris
import VCL_TOTAL.ABI.Foreign
...
#[link(name = "vcl_total")]
extern "C" {
    fn vcl_ut_init() -> *mut std::ffi::c_void;
    fn vcl_ut_free(handle: *mut std::ffi::c_void);
    fn vcl_ut_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = vcl_ut_init();
        assert!(!handle.is_null());

        let result = vcl_ut_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libvql_ut = "libvql_ut"

function init()
    handle = ccall((:vcl_ut_init, libvql_ut), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:vcl_ut_process, libvql_ut), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:vcl_ut_free, libvql_ut), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/vcl_total.h
   ```
